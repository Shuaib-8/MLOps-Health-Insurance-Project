...
def create_preprocessor(multi_cat_strategy='onehot'):
    """
    Create a preprocessing pipeline for health insurance data.
    
    Args:
        multi_cat_strategy: Strategy for multi-categorical encoding
            - 'onehot': OneHotEncoder (creates 4 columns)
            - 'ordinal': OrdinalEncoder (creates 1 column with 0,1,2,3)
    """
    logger.info(f"Creating preprocessor pipeline with {multi_cat_strategy} encoding for multi-categorical features")
    ...
    if multi_cat_strategy == 'onehot':
        multi_categorical_transformer = make_pipeline(
            OneHotEncoder(handle_unknown='ignore', sparse_output=False)
        )
    elif multi_cat_strategy == 'ordinal':
        multi_categorical_transformer = make_pipeline(
            OrdinalEncoder(handle_unknown='use_encoded_value', unknown_value=-1)
        )
    else:
...
def run_feature_engineering(input_file, output_file, preprocessor_file, encoding_strategy='onehot'):
    ...
    if strategy == 'target' and y is not None:
        # TargetEncoder needs the target variable during fitting
    ...
    if strategy == 'onehot':
        ...
        if len(preprocessor.transformers_) > 2:
            ...
            if hasattr(multi_cat_transformer, 'steps'):
                ...
                onehot_feature_names = onehot_encoder.get_feature_names_out(multi_cat_features)
    ...
    if hasattr(binary_encoder, 'categories_'):
        sex_mapping = {cat: idx for idx, cat in enumerate(binary_encoder.categories_[0])}
        smoker_mapping = {cat: idx for idx, cat in enumerate(binary_encoder.categories_[1])}
    ...
    if strategy == 'ordinal':
        ...
        logger.info(f"  Region Reverse (0=northeast, 1=northwest, etc.): {reverse_mapping}")
        ...
        if hasattr(onehot_encoder, 'categories_'):
            ...
            logger.info(f"  Created columns: {[name for name in feature_names if 'region_' in name]}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Feature engineering for health insurance data.')
